...
#define HOOKED_FUNC "puts"


typedef int (*old_native_function)(char* str) ;
old_native_function old_native_func;
...
typedef unsigned int (*OLDFUC)(char* str);
OLDFUC g_pfnOldAddr = NULL;
HOOK_INFO g_HookInfo;
int FLAG = 0;


int new_native_func(char* str)
{
    LOGD("This is new_native_func hook test");
    return old_native_func( "Hello CFCA!");
}
...
void WriteNewCode()
{
    memcpy((void *)g_HookInfo.m_nOldPfnAddr, (char*)g_HookInfo.m_nNewCode, sizeof(uint32_t) * 3);
...
int hook_native()
{
    ...
    old_native_func = (old_native_function) dlsym(handler, HOOKED_FUNC) ;
    if(!old_native_func)
    {
        printf("%s", dlerror());
        return -1;
